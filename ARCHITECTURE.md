...
- Browser/session operations require only the public Supabase URL and anon key, and admin authorization continues to use the verified session plus RLS. Contact, inquiry, and viewing inserts use one isolated server-only Supabase secret client after rate limiting, validation, and Turnstile; migration 020 denies direct anon/authenticated inserts and adds insert-only database invariants.
- Cloudflare Workers native Rate Limiting bindings protect contact/inquiry/viewing submissions at 5 requests per 60 seconds per IP and endpoint, the auth callback at 10 per 60 seconds per IP, favorites mutations at 60 per 60 seconds per verified user, property-image mutations at 10 per 60 seconds per verified admin, and other admin mutations at 60 per 60 seconds per verified admin. Login/signup also use Turnstile and Supabase Auth's own controls.
